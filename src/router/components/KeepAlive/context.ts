import type { KeepAliveContextType, RouteTransitionState } from './type'
import { createContext } from 'react'

/**
 * 创建一个相互隔离的 keep-alive 激活 / 失活回调注册表
 *
 * 每个 KeepAliveProvider 持有独立实例 → 不同 Outlet 层、不同 Router 之间即便产生同名
 * uniqueKey 也不会互相串扰（修复模块级全局单例导致的跨实例串扰）
...
/**
 * 向子树暴露当前所在 KeepAlive 实例的过渡状态（phase / finishEnter / finishExit）
 *
 * 未传 `transition` 时为 `null`，此时 {@link useRouteTransition} 返回 `null`，
 * 消费者应将其视为「无需关心过渡，正常渲染即可」
 */
export const RouteTransitionContext = createContext<RouteTransitionState | null>(null)
