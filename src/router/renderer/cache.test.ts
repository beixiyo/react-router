...
describe('updateCache', () => {
  ...
  it('应该添加新元素到缓存', () => {
    updateCache(cache, 'key1', mockElement, 10)
  ...
  it('应该在 effectiveLimit 为 undefined 时不添加缓存', () => {
    updateCache(cache, 'key1', mockElement, undefined)
  ...
  it('应该不更新已存在的缓存项', () => {
    ...
    updateCache(cache, 'key1', oldElement, 10)
    const oldEntry = cache.get('key1')

    const newElement = createElement('div', null, 'New')
    updateCache(cache, 'key1', newElement, 10)
  ...
  it('应该在超过限制时删除最久未使用的项', () => {
    ...
    // 添加第一个元素
    updateCache(cache, 'key1', element1, 3)

    // 添加第二个元素
    updateCache(cache, 'key2', element2, 3)

    // 添加第三个元素
    updateCache(cache, 'key3', element3, 3)

    expect(cache.size).toBe(3)

    // 添加第四个元素，LRUCache 会自动删除最久未使用的（key1，因为它是第一个添加的）
    updateCache(cache, 'key4', element4, 3)
...
describe('getCachedElement', () => {
  ...
  it('应该返回缓存中的元素', () => {
    cache.set('key1', { key: 'key1', element: mockElement, lastShown: Date.now() })
  ...
  it('应该更新 lastShown 时间戳', () => {
    ...
    cache.set('key1', { key: 'key1', element: mockElement, lastShown: initialTime })
