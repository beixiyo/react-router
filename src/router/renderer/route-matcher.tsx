 ...
 * @returns React 元素
 */
export function renderRouteChain(routeChain: RouteObject[], index: number = 0, match?: MatchResult): ReactElement {
  ...
  if (isLast) {
    return renderRouteComponent(route.component, params)
  }
  ...
  return (
    <OutletContext.Provider value={{ parentRoute: route, parentPath: route.path }}>
      <ParamsContext.Provider value={params}>
        { renderRouteComponent(route.component, params) }
 ...
 * @returns React 元素
 */
export function createRouteElement(route: RouteObject, match?: MatchResult) {
  // 如果有路由链，递归渲染整个路由链
  if (match?.routeChain && match.routeChain.length > 1) {
    return renderRouteChain(match.routeChain, 0, match)
  }
